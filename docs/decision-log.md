...
### `/enquete/` : une couche narrative, distincte des outils

**Situation :** Le site était jusqu'ici un lanceur d'outils (carte, audit, catalogue, rapports) pour un public data-literate — aucune couche de conclusions. Le classement des fiches produit pourtant trois constats grand public (routine, risques avérés, récidivistes).
**Décision :** Nouvelle page top-level `/enquete/` (pas une sous-page de `/rapports/`), findings-led : gros chiffre + barres CSS, chiffres **pré-calculés** en JSON au build (`build_public_angles.py`), CSP stricte locale (pas de sql.js, pas de carte, chargement instantané). Chaque angle « creuse » vers l'explorateur par un lien de hash. Les figures multi-label (M08, M04) sont comptées sur la table explosée `fiche_tags` (jamais sur les colonnes JSON-dans-VARCHAR — classe BUG-008) et réconciliées à des oracles indépendants qui échouent le build sur écart. Les récidivistes sont **nommés** publiquement, avec un disclaimer en tête (classement automatique, taux d'erreur, indicatif pas verdict) comme mitigation éditoriale suffisante.
**Justification :** Un lecteur grand public doit rencontrer les conclusions à une URL publique, pas dans un outil d'analyste titré « Angles d'analyse ». Le pré-calcul tient la promesse d'instantanéité (là où l'explorateur, motivé, tolère 78 Mo). La carte ne peut pas filtrer sur la gravité (données au niveau installation) — les liens « creuser » ciblent donc l'explorateur, seul propriétaire des axes de tags.
